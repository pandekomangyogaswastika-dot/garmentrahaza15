            ...
            <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-400/20">
              <div className="flex items-start gap-2 mb-2">
                ...
                <div className="flex-1">
                  ...
                  <select 
                    value={form.po_id} 
                    onChange={e => handlePOPick(e.target.value)} 
                    className="w-full border border-blue-400/30 bg-[var(--input-surface)] rounded-lg px-3 py-2 text-sm text-foreground"
                    data-testid="gr-po-select"
                  >
                    <option value="">Manual (tanpa PO)</option>
                    {purchaseOrders.map(po => (
                      <option key={po.id} value={po.id}>
                        {po.po_number} - {po.vendor_name} ({po.item_count} items)
                      </option>
                    ))}
                  </select>
                  {form.po_number && (
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <select
                          ...
                          onChange={e => handleMaterialPick(idx, e.target.value)}
                          className="w-full border border-blue-400/25 bg-[var(--input-surface)] rounded-lg px-2 py-1.5 text-xs text-foreground"
                          data-testid={`item-material-select-${idx}`}
                        >
                          <option value="">-- Tanpa link material --</option>
