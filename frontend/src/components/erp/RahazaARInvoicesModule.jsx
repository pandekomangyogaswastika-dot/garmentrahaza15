...
export default function RahazaARInvoicesModule({ token }) {
  ...
  const retryPost = async (id) => {
    setError('');
    const r = await fetch(`/api/rahaza/ar-invoices/${id}/post-to-gl`, { method: 'POST', headers });
    if (!r.ok) { setError(`Retry posting gagal (HTTP ${r.status})`); return; }
    const data = await r.json();
    if (data._posting_result && !data._posting_result.ok) {
      setError(`Posting masih gagal: ${data._posting_result.error}`);
    }
    fetchAll();
  };
      ...
      {loading ? <div className="flex items-center justify-center h-48"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-primary" /></div> : (
        <DataTable
          ...
          columns={[
            ...
            { key: 'gl_je_number', label: 'GL / JE', render: (r) => (
              r.gl_je_number ? (
                <span className="inline-flex items-center gap-1 text-[10px] font-mono text-emerald-300 bg-emerald-500/10 border border-emerald-500/30 px-1.5 py-0.5 rounded" data-testid={`ar-je-${r.invoice_number}`}>
                  <span>✓</span>{r.gl_je_number}
                </span>
              ) : r.post_error ? (
                <span className="inline-flex items-center gap-1 text-[10px] font-mono text-rose-300 bg-rose-500/10 border border-rose-500/30 px-1.5 py-0.5 rounded" title={r.post_error} data-testid={`ar-je-err-${r.invoice_number}`}>⚠ Error</span>
              ) : (
          ...
          rowActions={(r) => (
            <div className="inline-flex items-center gap-2">
              {r.status === 'draft' && <button onClick={() => sendInvoice(r.id)} className="text-xs text-[hsl(var(--primary))] hover:underline" data-testid={`ar-send-${r.invoice_number}`}>Kirim & Post</button>}
              ...
              {r.post_error && !r.gl_je_id && <button onClick={() => retryPost(r.id)} className="text-xs text-amber-300 hover:underline" data-testid={`ar-retry-${r.invoice_number}`}>Retry Post</button>}
            </div>
          )}
        />
      )}
