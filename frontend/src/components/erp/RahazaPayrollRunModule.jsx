...
import { Plus, RefreshCw, Download, CheckCircle2, Trash2, FileText, Eye, Lock, Calendar, DollarSign, AlertTriangle, CheckCircle, ShieldAlert } from 'lucide-react';
...
export default function RahazaPayrollRunModule({ token }) {
  ...
  return (
    <div className="space-y-5" data-testid="rahaza-payroll-run-page">
      ...
      {creating && <CreateRunModal onClose={() => setCreating(false)} onCreate={createRun} token={token} />}
      {viewing && <RunDetailModal data={viewing} token={token} onClose={() => setViewing(null)} onRefresh={() => openRun(viewing.run.id)} />}
    </div>
  );
}

function CreateRunModal({ onClose, onCreate, token }) {
  ...
  const sevColor = (sev) => sev === 'high' ? 'text-red-300 border-red-300/20 bg-red-400/8' : 'text-amber-300 border-amber-300/20 bg-amber-400/8';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <GlassCard className="p-6 max-w-xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          {/* Sprint 3.3: Attendance Validation */}
                ...
                {validation.summary.total_warnings === 0 ? (
                  ...
                  <>
                    <div className="flex items-center gap-2 text-xs text-amber-300 bg-amber-400/8 border border-amber-300/20 rounded-lg p-2.5">
                      ...
                      <span><strong>{validation.summary.total_warnings}</strong> karyawan punya attendance tidak lengkap ({validation.summary.warning_rate}% dari total). Payroll tetap bisa dijalankan.</span>
                    </div>
                    <div className="max-h-40 overflow-y-auto space-y-1">
                      ...
                      {validation.warnings.length > 10 && (
                        <p className="text-[10px] text-muted-foreground text-center">+{validation.warnings.length - 10} lainnya...</p>
                      )}
                    </div>
                  </>
                )}
