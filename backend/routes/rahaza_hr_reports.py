...
async def get_attendance_summary(
    ...
    total_alpha = sum(s["alpha"] for s in summary)
    ...
    current = d_from
    while current <= d_to:
        daily_counts[current.isoformat()] = {"hadir": 0, "izin": 0, "sakit": 0, "cuti": 0, "alpha": 0}
    ...
    for att in attendance_events:
        ...
        if d in daily_counts:
            if status in ("hadir", "present"):
                daily_counts[d]["hadir"] += 1
            elif status == "izin":
                daily_counts[d]["izin"] += 1
            elif status == "sakit":
                daily_counts[d]["sakit"] += 1
            elif status == "cuti":
                daily_counts[d]["cuti"] += 1
            elif status in ("alpha", "absent"):
                daily_counts[d]["alpha"] += 1
